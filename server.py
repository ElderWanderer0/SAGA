...
def register():
    ...
    try:
        ...
        if not is_admin:
            c.execute("INSERT INTO user_coupons (user_id, code, discount_rate) VALUES (?, ?, ?)",
                      (user_id, 'HOSGELDIN20', 0.20))
...
def add_art():
    ...
    conn = sqlite3.connect('users.db')
    conn.execute(
        "INSERT INTO artworks (user_id, title, description, price, image_path, category) VALUES (?, ?, ?, ?, ?, ?)", (
...
# --- YORUM VE DEĞERLENDİRME PUANLAMA / YANITLAMA API'LERİ ---

@app.route('/api/rate_comment', methods=['POST'])
def rate_comment():
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("INSERT INTO comment_ratings (user_id, comment_id, rating) VALUES (?, ?, ?) ON CONFLICT(user_id, comment_id) DO UPDATE SET rating=excluded.rating", (session['user_id'], request.json.get('id'), int(request.json.get('rating'))))
    conn.commit()
    conn.close()
    return jsonify({'success': True})

@app.route('/api/reply_comment', methods=['POST'])
def reply_comment():
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("UPDATE comments SET owner_reply=? WHERE id=?", (request.json.get('reply'), request.json.get('id')))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def rate_review():
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("INSERT INTO review_ratings (user_id, review_id, rating) VALUES (?, ?, ?) ON CONFLICT(user_id, review_id) DO UPDATE SET rating=excluded.rating", (session['user_id'], request.json.get('id'), int(request.json.get('rating'))))
    conn.commit()
    conn.close()
    return jsonify({'success': True})

@app.route('/api/reply_review', methods=['POST'])
def reply_review():
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("UPDATE reviews SET owner_reply=? WHERE id=?", (request.json.get('reply'), request.json.get('id')))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def add_review():
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("INSERT INTO reviews (user_id, item_type, item_id, review_text, event_rating) VALUES (?, ?, ?, ?, ?)", (session['user_id'], request.json.get('item_type'), request.json.get('item_id'), request.json.get('review'), int(request.json.get('event_rating', 5))))
    conn.commit()
    conn.close()
    return jsonify({'success': True})

@app.route('/api/get_reviews/<item_type>/<int:item_id>')
def get_reviews(item_type, item_id):
    conn = sqlite3.connect('users.db'); c = conn.cursor()
    c.execute('''SELECT r.id, r.user_id, r.review_text, r.owner_reply, r.created_at, u.fullname, u.profile_image, IFNULL((SELECT AVG(rating) FROM review_ratings WHERE review_id = r.id), 0) as avg_rating FROM reviews r JOIN users u ON r.user_id = u.id WHERE r.item_type=? AND r.item_id=? ORDER BY r.created_at DESC''', (item_type, item_id))
    reviews = [{'id': row[0], 'user_id': row[1], 'text': row[2], 'reply': row[3], 'created_at': row[4], 'author': row[5], 'img': row[6], 'rating': round(row[7], 1)} for row in c.fetchall()]
    conn.close()
...
def get_summary(item_type, item_id):
    ...
    try:
        c.execute(
            "SELECT SUM(IFNULL(total_price, tickets * ?)), SUM(tickets) FROM reservations WHERE item_type=? AND item_id=? AND payment_status='Ödendi'",
            (price, item_type, item_id))
    except sqlite3.OperationalError:
        c.execute(
            "SELECT SUM(tickets * ?), SUM(tickets) FROM reservations WHERE item_type=? AND item_id=? AND payment_status='Ödendi'",
            (price, item_type, item_id))
...
def get_chat(user_id=None):
    ...
    conn = sqlite3.connect('users.db')
    c = conn.cursor()
    c.execute("SELECT message, is_from_admin, created_at FROM live_chat WHERE user_id = ? ORDER BY created_at ASC",
              (target_user_id,))
...
def like_art():
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO likes (user_id, artwork_id) VALUES (?, ?)", (user_id, art_id))
        liked = True
    except sqlite3.IntegrityError:
        c.execute("DELETE FROM likes WHERE user_id=? AND artwork_id=?", (user_id, art_id))
        liked = False
    conn.commit()
...
def toggle_favorite():
    ...
    c = conn.cursor()
    try:
        c.execute("INSERT INTO favorites (user_id, artwork_id) VALUES (?, ?)", (user_id, art_id))
        favorited = True
    except sqlite3.IntegrityError:
        c.execute("DELETE FROM favorites WHERE user_id=? AND artwork_id=?", (user_id, art_id))
        favorited = False
    conn.commit()
    conn.close()
...
def process_payment():
    ...
    if p_type == 'art':
        ...
        final_price = art[1]
        if coupon_code:
            c.execute("SELECT discount_rate, is_used FROM user_coupons WHERE user_id = ? AND UPPER(code) = ?",
                      (user_id, coupon_code.upper()))
            coupon = c.fetchone()
            if coupon and coupon[1] == 0:
                final_price = art[1] * (1 - coupon[0])
                c.execute("UPDATE user_coupons SET is_used=1 WHERE user_id = ? AND UPPER(code) = ?",
                          (user_id, coupon_code.upper()))
        ...
        if res_data:
            ...
            c.execute("UPDATE reservations SET payment_status='Ödendi', total_price=? WHERE id=? AND user_id=?",
                      (final_price, p_id, user_id))
...
def delete_comment(cmt_id):
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("DELETE FROM comments WHERE id=? AND user_id=?", (cmt_id, session['user_id']))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def delete_review(rev_id):
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("DELETE FROM reviews WHERE id=? AND user_id=?", (rev_id, session['user_id']))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def delete_art(art_id):
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("DELETE FROM artworks WHERE id=? AND user_id=?", (art_id, session['user_id']))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def delete_workshop(w_id):
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("DELETE FROM workshops WHERE id=? AND user_id=?", (w_id, session['user_id']))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def delete_event(e_id):
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("DELETE FROM events WHERE id=? AND user_id=?", (e_id, session['user_id']))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def cancel_reservation(res_id):
    if 'user_id' not in session: return jsonify({'success': False})
    conn = sqlite3.connect('users.db')
    conn.execute("DELETE FROM reservations WHERE id=? AND user_id=?", (res_id, session['user_id']))
    conn.commit()
    conn.close()
    return jsonify({'success': True})
...
def apply_coupon():
    ...
    return jsonify(
        {'success': True, 'discount_rate': coupon[0], 'message': f'%{int(coupon[0] * 100)} indirim uygulandı!'})
...
def get_items_for_comparison(comp_type):
    ...
    if comp_type == 'art':
        c.execute(
            "SELECT a.id, a.title, a.price, a.category, u.fullname as artist FROM artworks a JOIN users u ON a.user_id = u.id")
        ...
        c.execute(
            "SELECT id, 'event' as type, title, price, capacity, e_date as date FROM events UNION SELECT id, 'workshop' as type, title, price, capacity, w_date as date FROM workshops")
        for row in c.fetchall():
            d = dict(row)
...
def get_content(page_name):
    ...
    if page_name == 'gallery':
        ...
        return jsonify({
                           ...
                           'sidebar': sidebar_links, 'is_admin': is_admin})
        ...
        try:
            c.execute(
                "SELECT id, item_type, item_id, tickets, payment_status, total_price FROM reservations WHERE user_id=? ORDER BY created_at DESC",
                (user_id,))
            reservations = c.fetchall()
        except sqlite3.OperationalError:
            c.execute(
                "SELECT id, item_type, item_id, tickets, payment_status, NULL as total_price FROM reservations WHERE user_id=? ORDER BY created_at DESC",
                (user_id,))
            reservations = c.fetchall()
        ...
        c.execute(
            "SELECT id, subject, message, admin_reply, status, created_at FROM support_tickets WHERE user_id = ? ORDER BY created_at DESC",
            (user_id,))
        tickets = c.fetchall()
        tickets_html = ""
        for t in tickets:
        ...
        user_list_html = "".join(
            [f"<li><strong>{u[0]}</strong> ({u[1]}) - <span style='color:#888; font-size:12px;'>{u[2]}</span></li>" for
             u in latest_users])
